...
local Set = {
	...
	Rebirthing = false,
	LayoutLoading = false,
	LayoutExpectedCount = 0, -- expected placed items after last successful load
...
local function ParseLayoutString(String)
	if typeof(String) ~= "string" or String == "" then
		return nil
	end
	local ok, layout = pcall(function()
		return game.HttpService:JSONDecode(String)
	end)
...
-- Wait until placed item count stops changing (layout finished streaming in)
local function WaitForLayoutSettle(timeout, minItems)
	...
	while os.clock() < deadline do
		...
		if c == lastCount and c >= minItems then
			stableSeconds += 0.2
			-- ~1s of no new items => treated as settled
			if stableSeconds >= 1 then
				return c
			end
		else
			stableSeconds = 0
...
local function IsLayoutFullyLoaded(tolerance)
	...
	local need = math.max(1, math.floor(expected * tolerance))
	return have >= need
end
...
local function LoadStringLayout(String)--Loads a layout from a string; waits until mostly placed
	...
	local expected = TableEntryCount(Layout)
	local base = GetPlayerTycoon()
	if not base or not base:FindFirstChild("Base") then
		return 0
	end
	...
	-- Wait for items to actually appear (string loads are async place spam)
	local settled = WaitForLayoutSettle(18, math.max(1, math.floor(expected * 0.5)))
	if Set.TestingMode then
		print("LoadStringLayout settled:", settled, "/", expected)
	end
	return expected
end

-- Loads one saved slot (Layout1/2/3) or "Layout String" and returns expected item count (best effort)
local function LoadOneLayout(layoutName)
	if not layoutName or layoutName == "None" then
		return CountPlacedItems()
	end
...
local function CollectBoxes()--Collectes all the boxes on the map
	-- Don't yank the player off-base while rebirthing / loading layouts
	if Set.Rebirthing or Set.LayoutLoading or Set.CollectingBoxes then
		return
	end
...
-- Forward-declared so the Auto Rebirth toggle can kick the loop immediately
...
local AutoRebithToggle = BoostPage:CreateToggle({
	...
	Callback = function(Value)
		...
		-- Kick the loop immediately when toggled on (don't wait for money Changed)
...
local MinWaitTime = BoostPage:CreateInput({
	...
	Callback = function(Text)
		...
		if Set.TestingMode then
			print("Rebirth Wait Min:", Set.MinWait)
		end
	end,
})

local MaxWaitTime = BoostPage:CreateInput({
	...
	Callback = function(Text)
		...
		if Set.TestingMode then
			print("Rebirth Wait Max:", Set.MaxWait)
		end
	end,
})

local WaitRangeHint = BoostPage:CreateParagraph({
...
function Load()--Loads the players layout(s) after rebirthing if auto rebirth is on
	...
	local ok, err = pcall(function()
		...
		-- Stay on base so farm boxes / leaving can't interrupt placement as easily
		TeleportToOwnBase()
		task.wait(0.15)
		...
		local function looksComplete(expected, fraction)
			...
			-- Saved layout with unknown size: need a real base, not an empty/interrupted one
			return have >= 5
		end

		if Set.TestingMode then
			print("Load First Layout:", Set.Layout1)
		end
		local exp1 = LoadOneLayout(Set.Layout1)
		expectedFinal = exp1 or 0

		-- Retry first layout once if almost empty (common when left base mid-load)
		if not looksComplete(expectedFinal, 0.5) then
			if Set.TestingMode then
				print("First layout incomplete (", CountPlacedItems(), "/", expectedFinal, "), retrying...")
			end
			TeleportToOwnBase()
			task.wait(0.1)
			game.ReplicatedStorage.DestroyAll:InvokeServer()
		...
		if Set.Layout2 ~= "None" then
			if Set.TestingMode then
				print("Using Second Layout:", Set.Layout2)
			end
			...
			-- If stacking layouts (no withdraw), expected is current total after L2 settles
			if Set.WithdrawBase then
			...
			if not looksComplete(expectedFinal, 0.85) then
				if Set.TestingMode then
					print("Second layout incomplete (", CountPlacedItems(), "/", expectedFinal, "), retrying final layout...")
				end
				TeleportToOwnBase()
				task.wait(0.1)
				if Set.WithdrawBase then
		...
		-- Final settle + record target for the watchdog
		...
		if Set.TestingMode then
			print("Layout load done. items=", CountPlacedItems(), " expected=", Set.LayoutExpectedCount, " verified=", Set.LayoutVerified)
		end
	end)
	-- Always clear loading flag so auto rebirth can never soft-lock
	...
	if Set.TestingMode then
		print("Layout Loading finished. ok=", ok)
	end
end

-- If base items dropped below the last good layout (e.g. leave-base interrupted load), reload it
local function EnsureLayoutLoaded()
	if Set.LayoutLoading or Set.Rebirthing then
		return
	end
	-- Only enforce while auto-farming so manual play isn't forced
	if not Set.AutoRebirth and not Set.OreBoost then
		return
	end
	if not Set.LayoutExpectedCount or Set.LayoutExpectedCount <= 0 then
		return
	end
	...
	-- Rate-limit reloads
	if os.clock() - (Set.LastLayoutReloadAt or 0) < 6 then
		return
	end

	local have = CountPlacedItems()
	-- After several failed attempts, accept current base so we don't loop forever
	-- (e.g. layout string items that can't place)
	if (Set.LayoutReloadFails or 0) >= 3 then
		if have > 0 then
			if Set.TestingMode then
				print("Layout reload gave up; accepting current count", have, "as expected")
			end
	...
	if Set.TestingMode then
		print("Layout incomplete (", have, "/", Set.LayoutExpectedCount, ") — reloading")
	end
	...
	-- Stacked L1+L2 needs the full Load path (both stages)
	if stackedLayouts then
		if Set.TestingMode then
			print("Stacked layouts incomplete → full Load()")
		end
		Load()
		if IsLayoutFullyLoaded(0.9) then
			Set.LayoutReloadFails = 0
		else
			Set.LayoutReloadFails = (Set.LayoutReloadFails or 0) + 1
		end
		return
	end

	-- Single final layout (or L2 with withdraw): soft reload final slot/string
	Set.LayoutLoading = true
	local ok, err = pcall(function()
		TeleportToOwnBase()
		task.wait(0.1)
		...
		if exp and exp > 0 then
			...
			if settled > (Set.LayoutExpectedCount or 0) then
				Set.LayoutExpectedCount = settled
			end
		end
	end)
	...
	if not ok then
		warn("EnsureLayoutLoaded soft reload error:", err)
	end
	...
	if IsLayoutFullyLoaded(0.9) then
		...
		if Set.AutoResizeUpgraders then
			pcall(ResizeUpgraders)
		end
		if Set.TestingMode then
			print("Soft layout reload OK. items=", CountPlacedItems())
		end
		return
	end

	-- Still incomplete → full two-layout Load path
	if Set.TestingMode then
		print("Soft reload failed, running full Load()")
	end
	Load()
	if IsLayoutFullyLoaded(0.9) then
		Set.LayoutReloadFails = 0
...
--[[
	Auto Rebirth engine
	- Does NOT require standing on your base (farm boxes can leave base freely)
	- Wait Min/Max is rolled AFTER layout load finishes (not on every money tick)
	- Flags always clear via pcall so it cannot soft-lock until manual reload
]]
local nextRebirthAt = 0 -- os.clock() timestamp; 0 = allowed immediately
...
local function isInfiniteMoney(value)
	...
	s = string.gsub(s, "[%s%$]", "")
	if s == "inf" or s == "+inf" or s == "-inf" or s == "infinity" then
		return true
	end
	if string.find(s, "inf", 1, true) then
		return true
	end
	return false
end
...
local function canAffordRebirth(money, price)
	if isInfiniteMoney(money) then
		return true
	end
	if type(money) ~= "number" or money ~= money then
		return false
	end
	if type(price) ~= "number" or price ~= price or isInfiniteMoney(price) then
		return false
	end
	return money > price
end
...
local function TryAutoRebirth()
	...
	-- Money / price (never require ActiveTycoon — farm boxes leaves base)
	local moneyVal
	local moneyOk, moneyErr = pcall(function()
	...
	local priceOk, RB = pcall(function()
		return RebornPrice(Player) * (1000 ^ (Set.Skips or 0))
	end)
	if not priceOk then
	...
	if not canAffordRebirth(moneyVal, RB) then
		return
	end
	...
	local ranOk, ranErr = pcall(function()
		...
		-- Poll for life change instead of a blind long wait
		...
		if Player.Rebirths.Value <= lifeBefore then
			if Set.TestingMode then
				warn("AutoRebirth: rebirth not confirmed, will retry after wait")
			end
		...
		if Set.TestingMode then
			print("AutoRebirth: rebirthed to life", Player.Rebirths.Value + 1)
		end
		...
		if not Set.AutoRebirth then
			return
		end

		-- Load layouts; wait range starts AFTER this finishes
		Load()
		if Set.AutoRebirth then
			scheduleNextRebirthWait("after-layout-load")
		end
	end)

	-- ALWAYS clear rebirth lock — this was the main "dies until reload" bug
	Set.Rebirthing = false
	Set.LayoutLoading = false
	if Set.OreBoost and Set.AutoRebirth then
		Set.OreBoostActive = true
	end
...
-- Money updates (works while finite; may stop firing at inf)
if Money then
	Money.Changed:Connect(function()
		if Set.AutoRebirth then
			RequestAutoRebirthCheck()
		end
	end)
end

-- Always-on watchdog: keeps auto rebirth alive with farm boxes, inf money, lag, etc.
-- Also re-loads layouts if leave-base / farm boxes interrupted placement.
task.spawn(function()
	while true do
		task.wait(0.35)
		if Set.AutoRebirth or Set.OreBoost then
			local okL, errL = pcall(EnsureLayoutLoaded)
			if not okL and Set.TestingMode then
				warn("EnsureLayoutLoaded error:", errL)
			end
		end
		if Set.AutoRebirth then
			RequestAutoRebirthCheck()
...
-- Safety: if flags ever stick for too long, force-clear them
task.spawn(function()
	...
	while true do
		...
		if Set.Rebirthing or Set.LayoutLoading then
			...
			-- Layout + second layout wait can be long; only force-clear after a long stall
			if os.clock() - stuckSince > 120 then
				...
				stuckSince = nil
				if Set.AutoRebirth then
					scheduleNextRebirthWait("force-unstuck")
				end
			end
		else
			stuckSince = nil
